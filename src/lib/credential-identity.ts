...
export interface RunCredentialPreflightArgs {
  ...
  iapubBaseUrl: string;
  /**
   * Pre-resolved PMAK identity (for example reused from validateApiKey's GET /me result).
   * When present the /me probe is skipped entirely; the identity is logged and cross-checked as-is.
   */
  pmak?: CredentialIdentity;
...
export async function runCredentialPreflight(args: RunCredentialPreflightArgs): Promise<void> {
  ...
  let pmak: CredentialIdentity | undefined = args.pmak;
  if (pmak) {
