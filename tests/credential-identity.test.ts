...
describe('credential identity', () => {
  ...
  it('runCredentialPreflight accepts a pre-resolved pmak identity (validateApiKey reuse) and skips the /me probe', async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async (input) => {
      const url = String(input);
      if (url.endsWith('/me')) {
        throw new Error('the /me probe must not run when pmak is pre-resolved');
      }
      return jsonResponse(sessionPayload(10490519));
    });

    await runCredentialPreflight({
      apiBaseUrl: API_BASE,
      iapubBaseUrl: IAPUB_BASE,
      pmak: pmakIdentity({ teamId: '10490519' }),
      postmanAccessToken: 'access-token-preresolved',
      mode: 'enforce',
      mask: passthroughMask,
      log: capture.log,
      fetchImpl
    ...
    const meCalls = fetchImpl.mock.calls.filter((call) => String(call[0]).endsWith('/me'));
    expect(meCalls).toHaveLength(0);
    expect(capture.infos.some((entry) => entry.includes('PMAK identity'))).toBe(true);
    expect(capture.infos.some((entry) => entry.includes('credential preflight OK'))).toBe(true);
  });
