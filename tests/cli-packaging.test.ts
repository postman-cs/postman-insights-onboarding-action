...
const EXPECTED_PACKAGE_NAME = '@postman-cse/onboarding-insights';
...
async function runWin32NativeShimPackaging(options: { executeCmd: boolean }): Promise<Win32NativeShimPlan> {
  const packDir = await makeTempDir('postman-insights-onboard-pack-');
  const extractRoot = await makeTempDir('postman-insights-onboard-extract-');
  const proxyRoot = await makeTempDir('postman-insights-onboard-proxy-');
...
describe('CLI packaging contract', () => {
  ...
  it(
    ...
    async () => {
      ...
      // Do not execute the native shim here; the platform packaging test owns Windows execution.
      expect(resolvePackStrategy('win32')).toBe('win32-native-shim');
      const plan = await runWin32NativeShimPackaging({ executeCmd: false });
      expect(plan.strategy).toBe('win32-native-shim');
