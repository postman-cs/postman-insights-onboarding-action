...
const CONFIG: RepoConfig = {"pkgName":"@postman-cse/onboarding-insights","binName":"postman-insights-onboard","pkgMain":"dist/index.cjs","actionMain":"dist/action.cjs","census":["action.cjs","cli.cjs","index.cjs"]};
...
async function writeFixture(root: string, options: FixtureOptions = {}): Promise<void> {
  ...
  if (options.contractEntry !== undefined) {
    await mkdir(path.join(root, 'scripts'), { recursive: true });
    await writeFile(path.join(root, 'scripts', 'dist-boot-contract.json'), JSON.stringify({ entry: options.contractEntry, exitCode: 0, outputIncludes: [] }), 'utf8');
  }
...
describe('verify-dist-artifact canonical contract', () => {
  ...
  it('fails the real verifier on the historical getter-only library export boot failure', async () => {
    const root = await makeTempDir('verify-dist-libboot-');
