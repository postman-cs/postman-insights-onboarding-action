 ...
 *
 * Pins the invariants whose absence burned release versions elsewhere: a burnt
 * version number must be skipped rather than reused, the tag must be created
 * only after committed release bytes pass every gate, and the workflow must
 * never push a commit onto protected main.
 */
...
describe('release-cut ordering contract', () => {
  ...
  it('does not import multifile receipt machinery', () => {
